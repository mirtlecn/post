...
import { getEmbeddedAssetUrl } from '../assets/index.js';
...
<footer class="markdown-body post-footer">
${footerHtml}
</footer>` : '';
    ...
    if (headingCount >= 2) {
      extraHead += `<link rel="stylesheet" href="${getEmbeddedAssetUrl('gfm_addons_css')}">\n`;
      extraBody += `<script src="${getEmbeddedAssetUrl('gfm_addons_js')}"></script>\n`;
    }
    ...
    if (htmlBody.includes('<code class="hljs language-') || htmlBody.includes('<code class="language-')) {
      extraHead += `<link rel="stylesheet" href="${getEmbeddedAssetUrl('highlight_light_css')}" media="(prefers-color-scheme: light)">\n`;
      extraHead += `<link rel="stylesheet" href="${getEmbeddedAssetUrl('highlight_dark_css')}" media="(prefers-color-scheme: dark)">\n`;
    }
...
<link rel="stylesheet" href="${getEmbeddedAssetUrl('ravel_gfm_css')}">
${extraHead}
<style>
  body {
    ...
    min-width: 200px;
    max-width: 838px;
    ${footerHtml ? 'min-height: 100vh;' : ''}
    margin: 0 auto;
    padding: 45px;
    ${footerHtml ? 'display: flex;' : ''}
    ${footerHtml ? 'flex-direction: column;' : ''}
  }
