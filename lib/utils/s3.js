...
// Do not switch these imports to createRequire as an EdgeOne build workaround.
// It can make the local EdgeOne build pass while production functions fail at
// startup with `Cannot find module '@aws-sdk/client-s3'` if dependencies are not
// packaged for the runtime. Fix EdgeOne packaging separately instead.

const DEFAULT_SIGNED_UPLOAD_URL_TTL_SECONDS = 300;
const DEFAULT_SIGNED_DOWNLOAD_URL_TTL_SECONDS = 300;
...
export function getSignedUploadUrlTtlSeconds() {
  return DEFAULT_SIGNED_UPLOAD_URL_TTL_SECONDS;
}

export function getSignedDownloadUrlTtlSeconds() {
  return DEFAULT_SIGNED_DOWNLOAD_URL_TTL_SECONDS;
}

export function getObjectKeyPrefix(ttl) {
  const oneDay = 86400;
...
export async function uploadFileToS3(file, ttl, contentType = file.mimetype) {
...
function buildCopySource(bucket, sourceKey) {
...
export async function generatePresignedPutUrl(
  objectKey,
  contentType,
  expiresSeconds = getSignedUploadUrlTtlSeconds(),
) {
  const client = getS3Client();
  const { bucket } = getConfig();
  const command = new PutObjectCommand({
...
export async function generatePresignedGetUrl(
  objectKey,
  expiresSeconds = getSignedDownloadUrlTtlSeconds(),
) {
  const client = getS3Client();
  const { bucket } = getConfig();
  const command = new GetObjectCommand({
