...
function hasFileContentLength(filePayload) {
  return Number.isSafeInteger(filePayload?.contentLength) && filePayload.contentLength >= 0;
}

async function resolveFileObjectMetadata(objectKey, metadata) {
  const normalizedMetadata = normalizeFilePayloadContentType(metadata);
  if (hasFileContentLength(normalizedMetadata)) {
    return normalizedMetadata;
  }

  return normalizeFilePayloadContentType(await headObject(objectKey));
}

function temporaryRedirectResponse(res, url) {
  res.writeHead(302, {
    Location: url,
...
export async function respondByType(req, res, { type, content, title = '', metadata = null, path, redis }) {
  ...
  try {
    const objectMetadata = await resolveFileObjectMetadata(content, metadata);
    if (isHeadRequest) {
      sendHeadResponse(res, {
    ...
    if (objectMetadata.contentLength > getCacheMaxBytes()) {
      temporaryRedirectResponse(res, await generatePresignedGetUrl(content));
      return;
    }
