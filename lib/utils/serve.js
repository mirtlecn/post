...
import { ensureUtf8CharsetForTextContentType } from './file-mime.js';
import { convertMarkdownToHtml, convertToQrCode } from './converter.js';
import { buildPublicLink } from './link-path.js';
import { getDomain } from './storage.js';
import { getTopicDisplayTitle, resolveTopicDisplayTitle, resolveTopicPath } from '../services/topic-store.js';

const TOPIC_CACHE_CONTROL = 'public, max-age=600, s-maxage=600';
const DEFAULT_CACHE_CONTROL = 'public, max-age=86400, s-maxage=86400';

function normalizeFilePayloadContentType(filePayload) {
  if (!filePayload) {
    return filePayload;
  }
  ...
  return {
    ...filePayload,
    contentType: ensureUtf8CharsetForTextContentType(filePayload.contentType),
  };
}

function buildCanonicalUrl(req, path) {
  if (!req || !path) {
    return '';
  }

  return buildPublicLink(getDomain(req), path);
}

export async function resolvePublicRender({ req, type, content, title = '', path, redis }) {
  const canonical = buildCanonicalUrl(req, path);
  ...
  if (type === 'topic') {
    return {
      responseKind: 'html',
      renderedContent: convertMarkdownToHtml(content, {
        pageTitle: resolveTopicDisplayTitle(path, { type, title }),
        canonical,
      }),
  ...
  if (type === 'md') {
    const renderOptions = { pageTitle: title, canonical };
...
export async function respondByType(req, res, { type, content, title = '', path, redis, raw = false }) {
  const isHeadRequest = req.method === 'HEAD';

  if (raw) {
    rawTextResponse(res, content, {
      head: isHeadRequest,
      cacheControl: type === 'topic' ? TOPIC_CACHE_CONTROL : DEFAULT_CACHE_CONTROL,
    });
    return;
  }
  ...
  if (type !== 'file') {
    let renderResult;
    try {
      renderResult = await resolvePublicRender({ req, type, content, title, path, redis });
    } catch (error) {
  ...
  try {
    ...
    if (cached) {
      const normalizedCached = normalizeFilePayloadContentType(cached);
      if (isHeadRequest) {
        sendHeadResponse(res, {
          contentType: normalizedCached.contentType,
          contentLength: normalizedCached.contentLength,
          cacheControl: 'public, max-age=86400, s-maxage=86400',
        });
        return;
      }
      binaryResponse(res, normalizedCached);
      return;
    }
  } catch (error) {
  ...
  try {
    const s3Object = normalizeFilePayloadContentType(await getS3Object(content));
    if (isHeadRequest) {
      sendHeadResponse(res, {
...
function rawTextResponse(res, content, { head = false, cacheControl = DEFAULT_CACHE_CONTROL } = {}) {
  const rawContent = typeof content === 'string' ? content : '';
  res.statusCode = 200;
  res.setHeader('Content-Type', 'text/plain; charset=utf-8');
  res.setHeader('Content-Length', Buffer.byteLength(rawContent, 'utf8'));
  res.setHeader('Cache-Control', cacheControl);
  res.end(head ? undefined : rawContent);
}
