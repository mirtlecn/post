...
function normalizeStoredMetadata(metadata) {
  ...
  if (Number.isSafeInteger(metadata.contentLength) && metadata.contentLength >= 0) {
    normalized.contentLength = metadata.contentLength;
  }
  if (typeof metadata.contentType === 'string' && metadata.contentType.trim()) {
    normalized.contentType = metadata.contentType.trim();
  }

  return Object.keys(normalized).length > 0 ? normalized : null;
}

export function buildStoredValue({ type, content, title = '', created = '', metadata = null }) {
  ...
  const normalizedMetadata = normalizeStoredMetadata(metadata);
  if (normalizedMetadata) {
    storedValue.metadata = normalizedMetadata;
  }
...
export function parseStoredValue(stored) {
  ...
  const normalizedMetadata = normalizeStoredMetadata(parsedValue.metadata);
  if (normalizedMetadata) {
    storedValue.metadata = normalizedMetadata;
  }
  return storedValue;
...
export function normalizeBaseDomain(value) {
  ...
  try {
    ...
    // Fall through for protocol-less values such as example.com/path.
  }

  input = input.replace(/^[a-zA-Z][a-zA-Z0-9+.-]*:\/+/, '');
  input = input.replace(/^\/+/, '');
  const host = input.split(/[/?#]/, 1)[0].trim();
...
export function getDomain(req) {
  const baseDomain = normalizeBaseDomain(process.env.BASE_DOMAIN);
  if (baseDomain) {
...
export function parseRequestBody(req) {
  return parseRequestBodyWithLimit(req, { maxBytes: DEFAULT_JSON_BODY_MAX_BYTES });
}
...
export function parseRequestBodyWithLimit(req, { maxBytes = DEFAULT_JSON_BODY_MAX_BYTES } = {}) {
  return new Promise((resolve, reject) => {
    ...
    function onData(chunk) {
      ...
      if (bodyBytes > maxBytes) {
        finishWithError(new RequestBodyTooLargeError(maxBytes));
        return;
      }
