...
async function handleTopicMutation(req, res, {
  ...
  if (existingStoredValue && !existingTopic) {
    return errorResponse(
      res,
      { code: 'conflict', message: `path "${path}" already exists`, hint: allowOverwrite ? undefined : 'Use POST /update to overwrite' },
      409,
    );
  }
  ...
  if (existingTopic && !allowOverwrite) {
    return errorResponse(
      res,
      { code: 'conflict', message: `path "${path}" already exists`, hint: 'Use POST /update to overwrite' },
      409,
    );
  }
...
async function handleFileUpload(req, res, { allowOverwrite }) {
  ...
  let normalizedTopic;
  try {
    const pathRequiredError = validateRequiredPathForUpdate(allowOverwrite, fields.path);
    ...
    if (allowOverwrite && fields.preservePath === 'true' && normalizedTopic) {
  ...
  const preservePath = allowOverwrite && fields.preservePath === 'true';
  const requestedPath = preservePath ? normalizedPath : buildUploadedFilePath(normalizedPath, fileExtension);
  try {
...
export function buildFileMetadataUpdate({ input, existingStoredValue }) {
  if (!existingStoredValue) {
    const error = new Error(`path "${input.path}" does not exist`);
  ...
  if (existingEntry.type !== 'file') {
    const error = new Error(`path "${input.path}" is not a file entry`);
  ...
  return {
    ...
    title: input.titleProvided ? input.title : existingEntry.title,
  };
}

async function handleFileMetadataUpdate(req, res, { input, isExport, requestReceivedAt }) {
  const pathValidationError = validateOptionalPath(input.path);
  if (pathValidationError) {
    return errorResponse(res, { code: 'invalid_request', message: pathValidationError }, 400);
  }

  if (input.topic) {
    return errorResponse(res, { code: 'invalid_request', message: '`topic` is not supported for file metadata updates' }, 400);
  }

  try {
    ensureCreatePathIsNotReserved(input.path);
  } catch (error) {
    return errorResponse(res, { code: 'invalid_request', message: error.message }, 400);
  }

  const redis = await getRedisClient();
  const existingStoredValue = await redis.get(`${LINKS_PREFIX}${input.path}`);
  let fileUpdate;
  try {
    fileUpdate = buildFileMetadataUpdate({ input, existingStoredValue });
  } catch (error) {
    return errorResponse(
      res,
      { code: error.code || 'invalid_request', message: error.message },
  ...
  let resolvedTopicPath;
  try {
    resolvedTopicPath = await resolveTopicPath(redis, { topicName: '', path: input.path });
  } catch (error) {
    return errorResponse(res, { code: 'invalid_request', message: error.message }, 400);
  }
...
async function handleJsonRequest(req, res, { allowOverwrite }) {
  ...
  const pathRequiredError = validateRequiredPathForUpdate(allowOverwrite, input.path);
  if (pathRequiredError) {
  ...
  if (allowOverwrite && input.inputType === 'file') {
    return handleFileMetadataUpdate(req, res, {
      input,
      isExport,
      requestReceivedAt,
    });
