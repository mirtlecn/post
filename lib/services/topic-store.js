...
export async function rebuildTopicIndex(redis, topicName, { title, titleProvided = false } = {}) {
...
export async function createTopic(redis, topicName, { title = '', titleProvided = false } = {}) {
  await ensureTopicItemsKey(redis, topicName);
  await adoptTopicItems(redis, topicName);
  await rebuildTopicIndex(redis, topicName, { title, titleProvided });
}

export async function refreshTopic(redis, topicName, { title = '', titleProvided = false } = {}) {
  await ensureTopicItemsKey(redis, topicName);
  await rebuildTopicIndex(redis, topicName, { title, titleProvided });
}
