...
export const MAX_FILE_SIZE_MB = parseFloat(process.env.MAX_FILE_SIZE_MB) || 3.5;
export const MAX_CONTENT_SIZE_KB = parseInt(process.env.MAX_CONTENT_SIZE_KB, 10) || 1024;
...
export function validateRequiredPathForUpdate(allowOverwrite, inputPath) {
  if (allowOverwrite && !inputPath) {
    return '`path` is required for update requests';
  }
...
export function validateRequestedUploadPath({ allowOverwrite, inputPath, inputTopic, originalFilename }) {
  const pathRequiredError = validateRequiredPathForUpdate(allowOverwrite, inputPath);
